...
// NewSessionStore inits and returns a new session store
func NewSessionStore(c *config.Config, db *Database) *gormstore.Store {
	fmt.Println(db.connection)
	store := gormstore.New((*db).connection, []byte(c.SessionSecret))
