...
func NewCustomContextHandler(db *gorm.DB, s *config.Config) *CustomContextHandler {
...
func (bc *BoatContext) ReturnView(code int, view string, data echo.Map) error {
	...
	if _, ok := data["title"]; !ok {
		data["title"] = bc.Config.SiteName
	} else {
		data["title"] = fmt.Sprintf("%s: %s", bc.Config.SiteName, data["title"])
	}

	// Set "siteName" if not already set
	if _, ok := data["siteName"]; !ok {
		data["siteName"] = bc.Config.SiteName
	}

	// Set "tagline" if not already set
	if _, ok := data["tagline"]; !ok {
		data["tagline"] = bc.Config.TagLine
	}
